...
// ReconcileDenormalized backfills the saves table's coins/farm_name columns
// for every row where they still look unpopulated — migration 002's zero
// defaults, or an import-v1 write that landed before this ran. It decodes
// each candidate blob with the ported sim and writes back only the
// denormalized columns, never the blob (the store never decodes blobs
// itself; that stays this package's job, since the state's shape is owned
// by internal/sim).
//
...
func ReconcileDenormalized(ctx context.Context, st *store.Store, logger *slog.Logger) error {
	...
	for _, row := range pending {
		...
		if err := st.BackfillDenormalized(ctx, row.Fingerprint, row.Slot, state.Coins, state.FarmName); err != nil {
